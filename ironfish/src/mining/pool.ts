 ...
 * file, You can obtain one at https://mozilla.org/MPL/2.0/. */
 ...
 export class MiningPool {
   ...
   async userHashRate(publicAddress: string | null): Promise<number> {
   ...
   async lucky(){
    const shareRate = await this.shares.shareRate()
    const currentHashrate = Number(BigInt(Math.floor(shareRate * decimalPrecision)) * this.difficulty) / decimalPrecision 
    return Number( this.difficulty / BigInt( currentHashrate ? currentHashrate : 1)  )  / decimalPrecision
  }
